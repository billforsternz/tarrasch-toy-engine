...
const char *cmd_go( const char *cmd )
{
    ...
    cprintf( "Input ms_time=%d, ms_inc=%d\n", ms_time, ms_inc );
    if( ms_time == 0 )
        ms_budget = 0;
    else if( ms_inc == 0 )
...
const char *cmd_position( const char *cmd )
{
    ...
    different_game = true;
    cprintf( "cmd_position(): Setting different_game = true\n" );
    ChessEngine tmp;
        ...
        if( s )
        {
            ...
            if( last_move_but_one.Valid() && last_move.Valid() )
            {
                ...
                if( prev_position == temp )
                {
                    ...
                    cprintf( "cmd_position(): Setting different_game = false\n" );
                }
            }
        }
    }
